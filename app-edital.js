...
// ===== POPULATE CONCURSO SELECT =====
function populateEditalConcursoSelect() {
    const select = document.getElementById('editalSelectConcurso');
    if (!select) return;
    select.innerHTML = '<option value="">Criar manualmente...</option>';
    concursos.forEach(c => {
...
// ===== SELECT CONCURSO → AUTO-FILL MATÉRIAS =====
document.getElementById('editalSelectConcurso')?.addEventListener('change', function() {
    const concursoId = parseInt(this.value);
    if (!concursoId) {
    ...
    if (!concurso) return;
    document.getElementById('editalNome').value = concurso.nome;
    currentEditalMaterias = concurso.disciplinas.map(d => ({
        nome: d.nome,
        topicos: []
    }));
    renderEditalTree();
    showToast(`${concurso.disciplinas.length} matérias carregadas do concurso "${concurso.nome}". Adicione os tópicos!`);
});
